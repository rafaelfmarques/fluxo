...
export default function TransactionsPage() {
  ...
  // Group transactions by date prefix (before comma)
  const groupedTransactions = MOCK_TRANSACTIONS.reduce((acc, tx) => {
    const [day, time] = tx.date.split(', ');
    if (!acc[day]) acc[day] = [];
    acc[day].push({ ...tx, time: time || '' });
    return acc;
  }, {} as Record<string, typeof MOCK_TRANSACTIONS[0][]>);
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-surface-card p-6 rounded-xl card-border flex items-center justify-between">
          ...
          <div className="bg-neon-lime/10 p-3 rounded-lg">
            <span className="material-symbols-outlined text-neon-lime">arrow_downward</span>
          </div>
        </div>
        <div className="bg-surface-card p-6 rounded-xl card-border flex items-center justify-between">
          <div>
          ...
          <div className="bg-neon-rose/10 p-3 rounded-lg">
            <span className="material-symbols-outlined text-neon-rose">arrow_upward</span>
          </div>
        </div>
        <div className="bg-surface-card p-6 rounded-xl card-border flex items-center justify-between">
          <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="flex flex-col gap-1.5">
            ...
            <div className="relative">
              <input className="w-full pl-10 pr-4 py-2 bg-background border border-border-subtle rounded-lg text-label-sm focus:ring-1 focus:ring-primary focus:border-primary text-on-surface outline-none" type="text" defaultValue="01 Out - 31 Out, 2023" />
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <select className="w-full px-4 py-2 bg-background border border-border-subtle rounded-lg text-label-sm focus:ring-1 focus:ring-primary focus:border-primary text-on-surface outline-none">
          ...
          <div className="flex items-end">
            <button className="w-full py-2 bg-primary/10 text-primary border border-primary/20 rounded-lg text-label-sm font-bold hover:bg-primary/20 transition-all flex items-center justify-center gap-2">
      ...
      <div className="bg-surface-card rounded-xl card-border overflow-hidden mb-12">
          ...
          <div key={day}>
            ...
            <div className="divide-y divide-border-subtle">
              {transactions.map((tx) => {
                ...
                // Determine badge color based on category (mocked logic)
                const badgeColor = tx.category === 'Alimentação' ? 'bg-orange-500/10 text-orange-400' :
                                   tx.category === 'Renda' ? 'bg-neon-lime/10 text-neon-lime' :
                                   tx.category === 'Transporte' ? 'bg-blue-500/10 text-blue-400' :
                                   tx.category === 'Lazer' ? 'bg-purple-500/10 text-purple-400' :
        ...
        <div className="p-6 flex justify-center border-t border-border-subtle bg-surface-card">
          <button className="flex items-center gap-2 text-primary text-label-sm font-bold hover:opacity-80 transition-all">
